...
- **Current batch:** Phase F done (modulo F1.6/F2.3); **G1 + G2.1/G2.3/G2.5 landed**. Next candidate: **G2.4** (proactive dialogue live in the interactive window over WS), then **G3**.
- **Last green commit:** `be4a13a` feat(demo): G2 — player-model panel, director-beat cue, deception tell.
- **Next:** **G2.4 then G3.** **G2.4 (EXP-209/210)** — proactive dialogue live in the INTERACTIVE window: an idle player gets hailed live (NPC-initiated line over WS). Pieces already exist — `demo_game/intent_ui.py`, `demo_game/proactive_dialogue_beat.py`, `client.get_pending_intents` (client.py:1369), and F1.2's `dialogue_ws` idle drain + `ProactiveQueue` deliver lines; EXP-225 built highlight/prefill. Inspect those first; G2.4 = wire the live hail (poll pending intents and/or consume the WS proactive_line push) into the interactive window with the existing highlight/prefill. Likely INLINE-able (small wiring) or a focused worker. Then **G3** — **G3.1** scripted "Intrigue" scenario (new `demo_game/scenarios/`, works under `--cinematic`) driving deception + (scheme when F1.6 lands) + player-model into one recordable arc; **G3.2** seed enrichment (⚠ `seed.py` → `make test-demo`) so the new panels are non-empty on fresh `make demo-seed` (scheme seeds need F1.6; player-model/deception/KNOWS_ABOUT can seed now). **After G (modulo G2.2 blocked) → Phase H** (economy/content/legacy — see §Ordered build plan H0–H3; H1 is the highest-value batch). **Original G2 batch notes below (for reference):**
