...
- SEV-04 GOSSIP+STORY_PACING+ROUTINE+SKILL DONE: `graph/routine_queries.py` created; `engines/routine/routine_queries.py` is now a thin re-export shim. `graph/skill_queries.py` gained `get_completed_quests_with_skills`; `skill_progression_engine` no longer calls `session.run` directly. rules_baseline: 48 (no change this batch). Remaining domains: idempotency, events, faction_politics, military, quest_generation, clique. Also retrieval/world/scheduler have raw Cypher — do these domain by domain.
