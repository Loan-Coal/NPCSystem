...
Layer: config
Purpose: Validates and normalises node property dicts against TypeRegistry models.
...
def validate_node_write(registry: TypeRegistry, node_type: str, props: dict[str, Any]) -> dict[str, Any]:
    """Validate and normalise a raw props dict[str, Any] against the registry model for node_type.

    If the node_type has no registered model (e.g. it is an unregistered ad-hoc type),
    the dict[str, Any] is returned as-is so callers are not broken by missing registrations.
    ...
    Args:
        ...
        props: Raw property dict[str, Any] to validate.

    Returns:
        Validated, None-stripped dict[str, Any] suitable for graph node writes.
