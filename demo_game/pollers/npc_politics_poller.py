"""
Module: npc_politics_poller
Layer: demo_game (external client — zero npc_engine imports)
Purpose: Background daemon thread that polls pledges and leverage for the active NPC
         on a fixed interval, exposing the latest snapshot thread-safely.
         Mirrors the NpcNeedsPoller / NpcGoalsPoller pattern; clears + re-polls
         instantly when the active NPC changes via set_active_npc().
...
class NpcPoliticsPoller:
    ...
    Calls ``client.get_pledges_for_npc`` and ``client.get_leverage_for_npc``
    every ``interval_s`` seconds. Results are stored under a lock and read
    via ``get_pledges()`` / ``get_leverage()``.

    Calling ``set_active_npc()`` clears cached data and triggers an immediate
    re-poll so the POLITICS panel updates instantly on NPC switch.
    ...
    def set_active_npc(self, npc_id: str | None) -> None:
        """Switch the target NPC, clear cached data, and trigger an immediate poll.
    ...
    def get_leverage(self) -> list[dict]:
        """Return the latest leverage snapshot for the active NPC.

        Thread-safe. Returns a copy so callers cannot mutate internal state.

        Returns:
            List of Leverage node dicts. Empty until the first successful poll.
        """
        with self._lock:
            return list(self._leverage)
    ...
    def _poll_once(self) -> None:
        """Fetch pledges + leverage for the active NPC and update shared state under lock.

        Silently swallows all exceptions. Discards results if the NPC switched mid-request.
        """
        with self._lock:
            npc_id = self._npc_id
        if npc_id is None:
        ...
        try:
            ...
            leverage = self._client.get_leverage_for_npc(npc_id)
            with self._lock:
