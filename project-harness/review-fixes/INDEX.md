...
- SEV-04 GOSSIP+STORY_PACING DONE: `graph/story_pacing_queries.py` created; engine now calls `get_active_high_severity_quests`/`get_recent_major_events`; `engines/story_pacing/pacing_queries.py` is now a thin backward-compat re-export. rules_baseline: 48 (was 50). Remaining domains: idempotency, events, faction_politics, military, quest_generation, skill, clique, routine. Also retrieval/world/scheduler have raw Cypher — do these domain by domain.
