...
## ISSUE-061: demo client ↔ API path drift — `/v1/pledges/...` 404 (pledges router mounted under `/v1/admin`)
**Found:** 2026-06-05, during demo-game regression triage (trade flow erroring)
**Severity:** P2 (demo politics/pledge panels error continuously; seed logs "Pledge … skipped … 404")
**Where:** `demo_game/client.py:1162,1182` (`GET /v1/pledges/characters/{id}`) vs `src/npc_engine/main.py:352` (`pledges_router` included with `prefix=admin_prefix` = `/v1/admin`) + `src/npc_engine/api/routes/pledges.py:63` (`prefix="/pledges"`). Actual route is `/v1/admin/pledges/characters/{id}`.
**Description:** The hardening refactor mounts `pledges_router` under `admin_prefix`, so the live path is `/v1/admin/pledges/...`. The demo client still calls `/v1/pledges/...` → FastAPI 404 `{"detail":"Not Found"}` (unregistered path, not a domain 404). Same drift is likely latent elsewhere: `client.py:830` calls `/v1/quests/offer` but `quest_generation_router` (`prefix="/quests"`) is under `admin_prefix` → `/v1/admin/quests/...`, while the live quest route is `/v1/quest/...` (singular). The demo's path map has diverged from the router mounts.
**Why deferred:** Logged as part of the Stage-0 demo-repair plan (EXPANSION_ROADMAP §Stage 0); fix alongside the full demo↔API endpoint audit so the whole contract is realigned at once, not piecemeal.
**To fix:** (1) Decide each drifted route's correct home — game-facing reads (pledges) arguably belong at `/v1`, admin-only at `/v1/admin`; simplest low-risk fix is to align the demo client to the live paths. (2) Audit ALL demo client paths against the live route table (cross-check the `client.py` path list vs `main.py` `include_router` prefixes). (3) Add a contract smoke test (see ISSUE-062 fix) that hits every demo endpoint so drift fails CI.
