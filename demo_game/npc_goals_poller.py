"""
Module: npc_goals_poller
Layer: demo_game (external client — zero npc_engine imports)
Purpose: Background daemon thread that polls Goal nodes for the active NPC on a
         fixed interval, exposing the latest snapshot thread-safely.
         Mirrors the NpcNeedsPoller pattern; switches instantly when the active NPC
...
class NpcGoalsPoller:
    """Background daemon that polls Goal nodes for the currently active NPC.

    Calls ``client.get_goals(npc_id)`` every ``interval_s`` seconds.
    Results are stored under a lock; read them with ``get_goals()``.

    Calling ``set_active_npc()`` clears cached data and triggers an immediate
    re-poll so the GOALS panel updates instantly on NPC switch.
    ...
    def set_active_npc(self, npc_id: str | None) -> None:
        """Switch the target NPC, clear cached goals, and trigger an immediate poll.
    ...
    def get_goals(self) -> list[dict]:
        """Return the latest goals snapshot for the active NPC.

        Thread-safe. Returns a copy so callers cannot mutate internal state.

        Returns:
            List of Goal node dicts. Empty until the first successful poll.
        """
        with self._lock:
    ...
    def _poll_once(self) -> None:
        """Fetch goals for the active NPC and update shared state under lock.
        ...
        if npc_id is None:
            return
        try:
            goals = self._client.get_goals(npc_id)
            with self._lock:
                if self._npc_id == npc_id:
                    self._goals = goals
        except Exception as exc:
            print(f"[NpcGoalsPoller] error: {exc}", file=sys.stderr)
