...
- **Current batch:** Phase F done (modulo F1.6/F2.3); G1, G2 (sans G2.2), **G3.2 landed**. Next candidate: **G3.1** (Intrigue scenario — the last Phase-G item before Phase H).
- **Last green commit:** `726cfe9` feat(demo): G3.2 — seed planted deception belief.
- **Next:** **G3.1 — the LAST Phase-G item.** Add a scripted "Intrigue" arc to the MAIN demo (`make demo-run`): the SCENES list lives in `demo_game/run.py:102`; Scene classes (NarratorCue, DialogueBeat, EventFire, ClockTick, ReputationDisplay, EmotionDisplay, …) live in `demo_game/run_scenes.py` (base `Scene` w/ `execute(runner)`, uses `runner.print_step/print_ok/client/dry_run`; tested in `test_run_scenes.py`). Add NEW display-style scenes driving **deception + player-model** (scheme deferred — F1.6): e.g. a `DeceptionRevealScene` (reads `client.get_beliefs("lira_fence")`, surfaces the `is_deception` belief seeded in G3.2 as a buyer "tell") + a `PlayerModelDisplayScene` (reads `client.get_player_model(npc, player_demo)` after a ClockTick so the F1.4 scheduler has produced data) + NarratorCue framing; insert into SCENES; must respect `runner.dry_run` + work under `--cinematic`. Add `test_run_scenes.py` cases (mock client, dry_run + populated paths). Likely INLINE (mirror `EmotionDisplay`/`ReputationDisplay`) or a focused worker. Exit: `make demo-run` plays the intrigue arc end-to-end. **After G3.1 → Phase G COMPLETE (modulo G2.2) → Phase H** (economy/content/legacy — §Ordered build plan H0–H3; **H1 `game_end_checker` economy rewrite is the single highest-value batch**, one worker, needs no F route). **Original G2 batch notes below (for reference):**
