...
- **ACTIVE: SEV-24 (Track D facade)** — `/fix-next` = next unchecked `SEV-24 ·` checkbox; brief `FIX-SEV-24.md` is self-contained. dialogue DONE (Wave 4): NEW `DialogueGraphPort` (archetype/voice/world_state/apply_relation_deltas/set_routine_override) + `Neo4jDialogueRepository` (session-per-call, first-contact retry logic); NEW `DialogueContextPort` + `Neo4jDialogueContextAdapter` (retrieval layer, holds GraphDB+embedding_index); `DialogueHandler.__init__` drops `session`/`embedding_index`, gains `dialogue_repo`/`dialogue_context`; `relation_mutator.apply_dialogue_relation_deltas` now takes `repo` not `session`; `build_dialogue_handler` sessionless; `dialogue_ws.py` drops manual `get_session()`. DEC-123 accepted R006 baseline (5 docstring-inflated functions). 2366 tests, make check GREEN. **NEXT: Wave 4 - quest cluster (12) / quest_generation cluster (10); then Wave 5 drops `session` from `BaseEngine.run_tick`/`tick_scheduler.advance()`.**
...
- [x] SEV-24 · gossip cluster (8 files) · dialogue cluster (6)
- [ ] SEV-24 · quest cluster (12) · quest_generation cluster (10)
