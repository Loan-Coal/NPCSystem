...
- SEV-04 GOSSIP DOMAIN DONE: `graph/labels.py`, `graph/relationships.py`, `graph/gossip_queries.py`, `graph/gossip_write_queries.py` created; gossip engine files updated. Remaining domains: idempotency, events, faction_politics, military, quest_generation, skill, clique, story_pacing, routine. Also retrieval/world/scheduler have raw Cypher — do these domain by domain.
