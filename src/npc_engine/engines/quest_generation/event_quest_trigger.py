...
from npc_engine.graph.event_trigger_queries import (
...
class EventQuestTrigger:
    ...
    async def run_tick(self, session: AsyncSession, tick_id: int) -> dict:
        ...
        """
        events = await get_unprocessed_trigger_events(
            session, self._trigger_event_types, _MAX_EVENTS_PER_TICK
        )
        quest_ids: list[str] = []
        for event in events:
            quest_id = await self._process_event(session, event, tick_id)
    ...
    async def _process_event(
        ...
        npc_id = await self._find_military_npc(session, location_id)
        if npc_id is None:
            _logger.warning(
    ...
    async def _find_military_npc(
        ...
        """
        if location_id:
            npc_id = await get_military_npc_at_location(
                session, location_id, self._military_archetypes
            )
            if npc_id is not None:
                return npc_id
        return await get_any_military_npc(session, self._military_archetypes)
