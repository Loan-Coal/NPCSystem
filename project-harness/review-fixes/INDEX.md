...
- SEV-04 GOSSIP+STORY_PACING+ROUTINE+SKILL+MILITARY+CLIQUE DONE: `graph/military_writer.emit_battle_event` created; `graph/group_queries` gained `get_high_affection_pairs/get_existing_shared_group/get_stale_cliques`; both engines no longer call `session.run` directly. rules_baseline: 46 (–2 this batch). Remaining domains: idempotency, events, faction_politics, quest_generation. Also retrieval/world/scheduler have raw Cypher — do these domain by domain.
