...
- **Current batch:** Phase F done (modulo F1.6/F2.3); **G1 + G2.1/G2.3/G2.4/G2.5 landed** (G2.2 blocked). Next candidate: **G3.1** (Intrigue scenario), then **G3.2** (seed enrichment).
- **Last green commit:** `80040c3` test(demo): G2.4 — lock proactive-hail path.
- **Next:** **G3 (the last Phase-G items).** **G3.1** — a scripted "Intrigue" scenario (new file under `demo_game/scenarios/` or `demo_game/run.py`'s scene system — check how `make demo-run` scenes work, e.g. `proactive_dialogue_beat.py` is a `Scene`) that drives **deception + player-model** (and scheme once F1.6 lands) into one recordable arc; must work under `--cinematic`. Exit: `make demo-run` plays the intrigue arc end-to-end. ⚠ scheme half needs F1.6 → scope G3.1 to deception + player-model now, leave a scheme hook. **G3.2** — seed enrichment (⚠ edits `demo_game/seed.py` → run `make test-demo`) so the new panels are non-empty on a fresh `make demo-seed`: a deception setup (an NPC BELIEVES with is_deception=true → feeds G2.5 tell + F3.3), player KNOWS_ABOUT already seeded (F3.6); scheme seeds need F1.6 (defer). Exit: panels non-empty on fresh seed. **After G3 (modulo G2.2 blocked) → Phase H** (economy/content/legacy — §Ordered build plan H0–H3; H1 `game_end_checker` economy rewrite is the single highest-value batch, one worker; needs no F route so can start once G is clear). **Original G2 batch notes below (for reference):**
