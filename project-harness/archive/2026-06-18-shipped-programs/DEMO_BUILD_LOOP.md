...
- **Phase in progress:** 🎉 **DONE — PHASES F + G + H ALL COMPLETE** (every in-scope item `[x]`). Only the DEC-107-deferred scheme trio (F1.6 / F2.3 / G2.2, human-gated) and the type-C pair (H-D1 / H-D2, out of overnight scope) remain. The autonomous F→G→H build loop has finished its full mandate.
- **Current batch:** — none — all batches landed. F + G(sans G2.2) + H0 + H1 + H2(.1–.8) + **H3(.1–.5)** green.
- **Last green commit:** `7d829b1` feat(H3): legacy gameplay-engine panels (oath/treaty/investigation/chapter/tension).
- **Next:** **H3 legacy panels** (all H0 enablers landed → unblocked): H3.1 oath panel+pledge_poller+break (H0.1), H3.2 treaty (H0.2), H3.3 investigation/contradiction (H0.3), H3.4 chapter/act (H0.4), H3.5 tension HUD (type-A). ⚠ the panel pollers + their `client.py` appends share files → **one worker for the H3 batch** (or serialize the client appends). New panel widget files are parallel-safe; each panel wires into `game_window`/`right_panel` (shared) so prefer ONE worker. After H3 → **demo-expansion DONE** (modulo F1.6-blocked scheme items F1.6/F2.3/G2.2 awaiting DEC-107) → wrap-up. **(H2.7 — DONE. Ref below.)** **H2.7 (DEMO-D2-08)** ⚠ de-hardcode `game_end_checker.py` win/lose constants to per-world (it has the DEC-108 waiver — careful): the current `DEMO_FACTIONS`/`WIN_STANDING_THRESHOLD`/`LOSE_LOCATION_ID`/`LOSE_FACTION_ID` are demo-world-specific; parameterize by a world id (a per-world config dict) so the Village/Tavern eval worlds are pickable+winnable. Then **H3 legacy panels** (all H0 enablers landed → unblocked): H3.1 oath panel+pledge_poller+break (H0.1), H3.2 treaty (H0.2), H3.3 investigation/contradiction (H0.3), H3.4 chapter/act (H0.4), H3.5 tension HUD (type-A); ⚠ poller/client appends share `client.py` → one worker for H3 (or serialize). **Deferred type-C (H-D1/H-D2) NOT in scope.** After H3 → demo-expansion DONE (modulo F1.6-blocked scheme items) → wrap-up. **(H1 + H2 — DONE except H2.6/H2.7. Ref below.)** **H2.8 (DEMO-D2-07)** — a `BranchBeat` Scene (in `demo_game/run_scenes.py` / `scenarios/`) that presents a `BranchNode` (the H2.1 garrick fork exists: `BRANCH_ID_GARRICK`, `branch_node.py`) during `make demo-run`, applies the chosen option's effects, and persists the choice to `BranchState` so `--cached` replay reproduces it; insert into a scenario beat list so a scripted scenario replays to a different outcome. INLINE-able (mirror the G3.1 Scene pattern + `proactive_dialogue_beat.py`); respect `dry_run`/`--cached`. **H2.6 (DEMO-D2-05)** — rival-quest variants + a `GameController` accept-guard (accepting one opposing-faction quest locks the other). **H2.7 (DEMO-D2-08)** ⚠ de-hardcode `game_end_checker.py` win/lose constants to per-world (touches the DEC-108-waived file — careful) so Village/Tavern eval worlds are pickable+winnable. Then **H3 legacy panels** (all H0 enablers landed → unblocked): H3.1 oath panel+pledge_poller+break (H0.1), H3.2 treaty (H0.2), H3.3 investigation/contradiction (H0.3), H3.4 chapter/act (H0.4), H3.5 tension HUD (type-A); ⚠ poller/client appends share `client.py` → one worker for H3 client appends or serialize. **Deferred type-C (H-D1/H-D2) NOT in scope.** After H2 + H3 → demo-expansion DONE (modulo F1.6-blocked scheme items) → wrap-up. **(H1 + H2-content + H2.1 — DONE. Ref below.)** **H2.1 (DEMO-D2-06)** = NEW files (`demo_game/branch_node.py` + `branch_state.py` + `branch_effects.py` — typed effects belief/rep/world-state/quest, OCP one-file-per-effect — + `ui/branch_panel.py`), resolving choices over existing client methods + H0.5 `post_quest_choice`; a player choice forks outcomes. New-file work → INLINE or a focused worker; pure demo-side. **H2.8** (BranchBeat in `scenarios/` over H2.1 + persisted BranchState) folds in. **H2.6** = rival-quest variants + `GameController` accept-guard (can't accept opposing-faction quests simultaneously). **H2.7** ⚠ de-hardcodes `game_end_checker.py` win/lose constants to per-world (touches the DEC-108-waived file — careful) so Village/Tavern eval worlds become playable. Then **H3 legacy panels** (all H0 enablers landed → unblocked): H3.1 oath panel+pledge_poller+break (H0.1), H3.2 treaty panel (H0.2), H3.3 investigation/contradiction panel (H0.3), H3.4 chapter/act panel (H0.4), H3.5 tension HUD (type-A); ⚠ poller/client appends share `client.py` → one worker for the H3 client appends or serialize. **Deferred type-C (H-D1/H-D2) NOT in scope.** After H2 + H3 → demo-expansion DONE (modulo F1.6-blocked scheme items) → wrap-up. **(H1 economy + H2 content — DONE. Ref below.)** **H2 (DEMO-D2 mini-specs)** — ⚠ H2.2–H2.5 edit `seed.py`/`seed_npc_data.py` → **ONE seed worker** (more NPCs/locations/branching content so the game has depth; H2.1 branch primitive = new files, H2.8 depends on it → same/earlier cycle; H2.6 `GameController` guard + H2.7 `game_end_checker` per-world de-hardcode ⚠ touches game_end_checker again → AFTER H1, careful w/ DEC-108). Pure demo-side → `make test-demo` (+ `make check` if any src). Scheme content needs F1.6 (defer). **H3 (legacy panels)** — each consumes an H0 enabler (ALL H0 landed → unblocked): new panel files parallel-safe; poller/client appends ⚠ `client.py` (one worker for client appends). H3.5 tension HUD is type-A (no enabler). H3 panels: oath-break (H0.1), treaty (H0.2), investigation/contradiction (H0.3), chapter/act (H0.4). **Deferred type-C (H-D1 military sim, H-D2 server-side decrement) NOT in scope.** Start: read `demo-expansion/CONTENT_PLAN.md` + `DEMO_EXPANSION_ROADMAP.md` for the H2/H3 `DEMO-Dx` item list, then dispatch the H2 seed/content worker (shared `seed.py`). After H2 + H3 → **demo-expansion DONE** (modulo F1.6-blocked scheme items) → loop DONE/wrap-up. **(H1 economy — DONE. Ref below.)** Rewrite `demo_game/game_end_checker.py` to a multi-objective win/lose economy per the `DEMO-D3` mini-specs in `demo-expansion/`: **H1.1** multi-objective win (faction-standing OR wealth OR quest-chain OR brokered treaty via H0.2); **H1.2** currency win/lose axis (`WEALTH_WIN_THRESHOLD`; bankruptcy `BANKRUPTCY_LOSE_THRESHOLD` armed after gold was once positive) over `GoldPoller`; **H1.3** faction tension/overreach (gains with one faction cost a rival via `adjust_npc_reputation`, `client.py:~1414`) as a branch/quest effect; **H1.4** tick-deadline pressure (relative `DEADLINE_TICKS` from a latched start tick via `get_clock_state().current_tick`); **H1.5/H1.6** end-card UI (`demo_game/ui/`) + objective-tracker poller. **Constraint:** keep `evaluate_game_end` ≤40 lines / ≤3 nesting — extract `check_win_multi`/`compute_grade`/failure-selection helpers (named constants in `demo_game/constants.py`, no magic numbers). Baseline verified 2026-06-12: `game_end_checker.py` still single-win (2/3 factions ≥ 50) + inert single-lose. Needs NO F route → start now. Inspect `game_end_checker.py` + `game_end_poller.py` + `gold_poller.py` first. After H1 → **H2 content** (seed worker) → **H3 legacy panels** (each gated on its H0 enabler, now all landed). **(H0 enablers FIRST — DONE. Ref below.)** Per §Ordered build plan + invariant 3: ~~H0 enablers FIRST~~ (the only non-pure-demo items — small engine-side route/client additions, DEC-free): **H0 = ONE worker for all five** (H0.1 `EngineClient.break_pledge`, H0.2 `create_treaty`/`get_faction_treaties`/`break_treaty`, H0.3 new read-only `api/routes/investigations.py` + `get_investigation`, H0.4 new `api/routes/chapters.py` + `get_current_chapter`, H0.5 `post_quest_choice`) — `client.py` is the shared file so all five go to one worker; lands 2 new route files. **H0 touches `src/` → run `make check` AND `make test-demo`.** Then **H1 economy** (the single highest-value batch — "it's a game now"): ONE worker rewrites `demo_game/game_end_checker.py` to multi-objective win (faction OR wealth OR quest-chain OR treaty) + currency win/lose + tension/overreach + tick-deadline, keeping `evaluate_game_end` ≤40 lines (extract `check_win_multi`/`compute_grade`/failure-selection) + end-card UI + objective-tracker poller; needs no F route. Then **H2 content** (one seed worker for `seed.py`/`seed_npc_data.py` + branch primitive), **H3 legacy panels** (each gated on its H0 enabler). **Deferred type-C (H-D1/H-D2) NOT in scope.** Start: dispatch the H0 worker (all 5, shared `client.py`). **Original G2 batch notes below (for reference):**
...
- **23 · 2026-06-12 H1 (economy, all 6)** — PASS. The "it's a game now" batch (one worker, pure demo-side).
  Rewrote `game_end_checker.py` to multi-objective: `check_win_multi` (faction OR wealth≥500 OR quest-chain≥3
  OR treaty) → `win_path`; `check_lose_bankrupt` (armed only after gold>0 via `_seen_positive_gold` latch in
  the poller), `check_lose_deadline` (relative `DEADLINE_TICKS=40` from a `_start_tick` latch), `check_overreach`
  (rival-floor win-blocker); `_select_failure` priority chain (legion>bankruptcy>deadline>overreach, lose beats
  win); `compute_grade` S/A/B/C; `ObjectiveState` gains win_path/failure_reason/total_gold/ticks_remaining/grade;
  end-card overlay renders path+grade+stats; constants migrated to `constants.py`. `evaluate_game_end` kept ≤40
  lines (helpers extracted). Cherry-picked `596f7e1`; integration: DEC-108 300-line waiver for the cohesive
  evaluator (343 lines) + ratcheted baseline (`8761f82`). +63 checker tests. Gates: `make check` 2115 +
  `make test-demo` 882. Server-side rival decrement deferred (type-C, per spec). **→ advancing to H2 content.**
- **24 · 2026-06-12 H2.2–H2.5 (content)** — PASS. One seed worker grew the world: 8→14 NPCs (6 new, payload
  data split into new DATA-ONLY `seed_npc_data.py`), +4 venues + 2 districts (PART_OF tiers via `post_part_of`),
  3→5 factions (2 new alliable-but-not-win-eligible — `DEMO_FACTIONS` stays 3 to avoid touching game_end_checker;
  per-world win sets are H2.7), 6→18 quests across 6 chains (chain-successor IDs added to `WIN_QUEST_CHAIN_IDS`
  → 11 total, feeding H1.1's quest-chain win at `QUEST_CHAIN_WIN_COUNT=3`). All idempotent (KE-6). Cherry-picked
  `e290d70`; integration: DEC-109 300-line waiver for the data-only `seed_npc_data.py` (465 lines) + ratchet
  (`1a05313`). +40 seed tests. Gates: `make check` 2115 + `make test-demo` 912. **→ H2.1 branch primitive next.**
- **25 · 2026-06-12 H2.1 (branch primitive)** — PASS. The keystone fork construct (one worker, new files):
  `branch_node.py` (`BranchNode`/`BranchOption` frozen), `branch_effects.py` (OCP: `BranchEffect` Protocol +
  5 frozen effect classes each `apply(client)` over EXISTING client methods — RepDelta→`adjust_npc_reputation`,
  SetBelief→`post_belief`, WorldState→`put_world_state`, OfferQuest→`post_quest_choice`, GotoBeat=no-op),
  `branch_state.py` (immutable `BranchState` + JSON persist to `.cache/demo/`, sole source of truth for
  `--cached` replay), `ui/branch_panel.py` (modal choice widget). First slice = garrick spare/turn-in 2-option
  fork (opposite RepDeltaEffects), tested end-to-end. Cherry-picked `948c132`; DEC-110 waiver for branch_panel
  (346 lines) + ratchet (`6bca4e5`). +60 tests. Gates: `make check` 2115 + `make test-demo` 972. **→ H2.8 next.**
- **26 · 2026-06-12 H2.8 (BranchBeat)** — PASS. Scripted fork over the H2.1 primitive: added `build_garrick_branch()`
  factory (spare→+thieves_guild, turn-in→+city_guard via RepDeltaEffect) + a `BranchBeat` Scene in `run_scenes.py`
  that renders the fork in `make demo-run`, resolves the choice (prior `BranchState` wins → reproducible `--cached`
  replay; else `option_index`), applies the chosen option's effects to the live graph, persists `BranchState`;
  wired into ACT 12. Inline; +5 tests. Gates: `make check` 2115 + `make test-demo` 977. **→ H2.6/H2.7 next.**
- **27 · 2026-06-12 H2.6 (rival-quest accept-guard)** — PASS. Added `RIVAL_QUEST_PAIRS` (city-guard
  `bren_deliver_ore` vs thieves-guild `tilda_gather_herbs`) + a `QuestTradeController` accept-guard:
  `_rival_already_accepted` checks the pairs against `_accepted_quest_ids`; `on_quest_accept` refuses
  (status message) when a rival is already accepted, else accepts + records. Inline; +2 tests. Gates:
  `make check` 2115 + `make test-demo` 979. **→ H2.7 (per-world game_end constants) — last H2 item.**
- **29 · 2026-06-13 H3 (all 5 legacy panels) — FINAL BATCH** — PASS. One worktree worker (shared
  `game_window`/`right_panel` wiring; all required `client.py` methods already landed in H0 → client.py
  untouched). H3.1 oath panel + `pledge_poller` (swear/list/break over `post_pledge`/`get_pledges_for_npc`/
  `break_pledge`); H3.2 treaty board + `treaty_poller` (merges all-faction treaties, broker/break); H3.3
  investigation "solve-the-crime" panel over `get_investigation` (alibi/rumor contradictions + graph
  provenance — **scheme-discovery overlay skipped, deferred to F2.3/DEC-107**); H3.4 chapter banner +
  `chapter_poller` + `_draw_chapter_banner()` HUD; H3.5 tension HUD + `tension_poller` +
  `_draw_tension_hud()` over `get_world_state` severity/quest-rate. 3 new RightPanel tabs (17→20) + 2 HUD
  overlays. 14 new files (4 pollers, 3 panels, 7 test files) + 3 modified (right_panel, game_window,
  test_right_panel). Cherry-picked `2ccaa05` clean (merge-first held) → integration commit `7d829b1`; pure
  demo-side (zero `src/` imports). +79 tests. Gates: `make test-demo` 1068 + `make check` 2115 passed,
  27 skipped, check-rules OK (143 grandfathered — no new violations; `right_panel`/`game_window` already
  baselined). **→ 🎉 DEMO-EXPANSION DONE. F→G→H autonomous loop COMPLETE.**

- **★ FINAL · 2026-06-13 — F→G→H BUILD LOOP COMPLETE.** 29 cycles, ~45 ROADMAP items landed green on
  `munich-demo`, zero pushes (local-only per invariant 1). **Phase F** (activate & expose): relationship-phase
  transitions, proactive-dialogue delivery, config-selectable emotion model, player-model/director/
  memory-decay scheduler ticks, 5 new read routes, gossip Standing-gated secret-share, dialogue_turn
  session persistence (DEC-106 schema), anti-hallucination deception rescue. **Phase G** (surface cognition):
  facial-glyph/retrieval/breadcrumb/relationship-phase panels, player-model + director-beat + deception-tell
  panels, intrigue scenario + seed enrichment. **Phase H** (make it a game): H0 legacy-engine enablers, H1
  multi-objective economy (faction/wealth/quests/treaty win + 4 lose paths + grades), H2 content (14 NPCs,
  7 locations, 5 factions, 18 quests, branching forks, rival-quest exclusivity, per-world objectives), H3
  legacy panels (oath/treaty/investigation/chapter/tension). **Deferred for a human DEC-107 call:** F1.6
  scheming-advance (`add_scheme_step` MERGEs a bare invalid Event — the one design decision the loop refused
  to smuggle), which blocks F2.3 + G2.2. **Out of overnight scope (type-C, OQ-5/OQ-6):** H-D1 military sim,
  H-D2 server-side standing decrement. Final gate green: `make check` 2115 passed / 27 skipped / 86.51% cov,
  `make test-demo` 1068. No wake scheduled — loop stopped per §Done.
- **28 · 2026-06-13 H2.7 (per-world game_end objectives)** — PASS. De-hardcoded `game_end_checker.py`
  from the single demo world: new `demo_game/world_objectives.py` with a frozen `WorldObjectives` bundle
  (factions, win threshold/min, lose location+antagonist, wealth/bankruptcy, quest-chain, deadline, rivals)
  + `WORLD_OBJECTIVES` registry keyed by world id (`demo`/`village`/`tavern`). Every checker predicate and
  `GameEndPoller` gained a trailing `objectives: WorldObjectives = DEMO_OBJECTIVES` param (backward-compatible
  — demo tests unchanged). Village (`vw_village_council`/`vw_farmers`) + Tavern (`tw_merchants`/`tw_innkeepers`)
  are now winnable via a two-faction path; empty `lose_location_id`/`lose_faction_id` disables the legion lose
  (no antagonist), empty `win_quest_chain_ids` disables the quest path. Poller skips the CONTROLS poll when no
  antagonist. **Avoided the DEC-108 trap:** instead of growing the waived `game_end_checker.py` further,
  extracted the objectives into the new `world_objectives.py` — that tipped `constants.py` over 300 lines, so
  moved the whole bundle there (constants back under 300, **no new waiver**). Repointed `world_panel.py`
  `DEMO_FACTIONS`/`WIN_STANDING_THRESHOLD` imports to their canonical `constants.py` home. Inline; +10 tests.
  Gates: `make check` 2115 + `make test-demo` 989. **→ H2 COMPLETE. Next: H3 legacy panels (final batch).**
