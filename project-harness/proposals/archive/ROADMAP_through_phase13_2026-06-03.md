...
- [~] **S13.2** ISSUE-051 — runtime config mutation. **DROPPED 2026-06-03** (user decision): scheduler-loop
  + public-endpoint change deprioritized below the review-remediation backlog. ISSUE-051 closed won't-fix;
  dashboard controls stay read-only. See DEC-055.
- [x] **S13.3** Doc-drift sweep. Found one genuine stale-stub docstring: `engines/military/__init__.py`
  described the (now-implemented, S6.5) military engine as a "no-op tick placeholder." Corrected.
  Other hits (placeholder secrets, unimplemented Qdrant backend, clique tick-throttle skip) are accurate, not drift.
  - Exit met: no docstring describes shipped behavior as a stub.
...
| S13.3 | 2026-06-03 | Phase 13 | Doc-drift sweep: corrected `engines/military/__init__.py` stale "no-op stub" docstring (military implemented S6.5). S13.2 dropped (DEC-055, ISSUE-051 won't-fix). Phase 13 complete | Phase 13 closed |
| Restructure | 2026-06-03 | Phase 13 | Archived full roadmap (Phases 0–13) → `proposals/archive/ROADMAP_through_phase13_2026-06-03.md`; slimmed live ROADMAP.md to Phase 14 onwards + review-remediation pointer | This archive snapshot |
