...
- **EXP-00b — fix demo↔API path drift (ISSUE-061, P2).** `/v1/pledges/...` 404s because pledges is now mounted at `/v1/admin/pledges/...`; `/v1/quests/offer` is similarly suspect. Realign the client paths (or re-home game-facing routes at `/v1`) after a full audit of every `client.py` path vs the live `include_router` mounts.
- **EXP-00c — boot + demo-endpoint smoke test in CI.** A test that boots the stack and hits every endpoint the demo calls (asserting non-5xx / non-404), so path drift and construction errors fail CI. Closes the long-standing "live-only breaks are unguarded" gap (L9-01/02/05, SEV-02).
